...
namespace Blocks
{
    public class CloakingDeviceMod : BlockMod
    {
        ...
        public override string Name { get { return "Cloaking_Field_Generator_Mod"; } }
        public override string DisplayName { get { return "Cloaking Field Generator Mod"; } }
        ...
        protected Block PrototypeCloakDevice = new Block()
            ///模块ID
            .ID(529)

            ///模块名称
            .BlockName("Prototype Cloaking Device​")

            ///模型信息
            .Obj(new List<Obj> { new Obj("Cloak Device.obj", //Obj
                                         "PRO Cloak Device Texture.png", //贴图
                                         new VisualOffset(new Vector3(1f, 1f, 1f), //Scale
                                                          new Vector3(0f, 0f, 0f), //Position
                                                          new Vector3(0f, 0f, 0f)))//Rotation
            })
        ...
        public override void OnLoad()
        {
            ...
            LoadBlock(PrototypeCloakDevice);
    ...
    public class PROCloakScript : BlockScript
    {
        protected MKey Key1;
        ...
        protected override void OnSimulateStart()
        {
            ...
            FieldDetector.transform.position = this.transform.position;
            Destroy(FieldDetector.GetComponent<Renderer>());
            FieldDetector.AddComponent<StealthCloakFieldScript>();

            foreach (MeshRenderer REDer in Machine.Active().SimulationMachine.GetComponentsInChildren<MeshRenderer>())
            {
                if (!REDer.GetComponent<BeingPROCloakedScript>())
                {
                    REDer.gameObject.AddComponent<BeingPROCloakedScript>();
                    REDer.gameObject.GetComponent<BeingPROCloakedScript>().InvisibleBumpedMat = (InvBumpTex);
                    REDer.gameObject.GetComponent<BeingPROCloakedScript>().InvisibleBumpedMat.mainTexture = (InvBumpTex.mainTexture);
                    REDer.gameObject.GetComponent<BeingPROCloakedScript>().InvisibleMat = (InvTex);
                    REDer.gameObject.GetComponent<BeingPROCloakedScript>().InvisibleMat.mainTexture = (InvTex.mainTexture);
                    //REDer.gameObject.tag += "Cloaked";
                }
            }
    ...
    public class BeingPROCloakedScript : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            {
                Cloaking(渐变数值 * 2, OrigColor, BumpOFFSET);
                if (!this.GetComponentInParent<MachineTrackerMyId>().gameObject.name.Contains("IsCloaked"))
                    this.GetComponentInParent<MachineTrackerMyId>().gameObject.name += "IsCloaked";
            }
            else
            {
                Decloaking(渐变数值 * 2, OrigColor);
    ...
    public class StealthCloakFieldScript : MonoBehaviour
    {
        ...
        void OnTriggerStay(Collider col)
        {
            try
            {
                foreach (BeingPROCloakedScript cloakingObject in col.gameObject.GetComponentInParent<MachineTrackerMyId>().GetComponentsInChildren<BeingPROCloakedScript>())
                {
                    cloakingObject.InsideAField = true;
                }
            }
